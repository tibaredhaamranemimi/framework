...
{ 
	...
	function datediffUS($datejour1,$datejour2,$type)
    {
	$Timestamp1 = $this->CalculateTimestampFromCurrDatetime($datejour1);
	$Timestamp2 = $this->CalculateTimestampFromCurrDatetime($datejour2);
	$DateDiff = $this->CalculateDateDifference($Timestamp1, $Timestamp2);
	...
	function listenatalite($STRUCTURED,$datejour1,$datejour2,$login,$type)
	{
	...
	$this->SetXY(5,15);$this->cell(290,5,html_entity_decode(utf8_decode($this->dspfr.': '.$this->nbrtostring("wil","IDWIL",$this->nbrtostring("structure","id",$STRUCTUREDX[1],"idwil"),"WILAYAS"))),0,0,'C',1,0);
	if($STRUCTURED=='IS NOT NULL') {
	$this->Text(05,25,html_entity_decode(utf8_decode("Service Prévention")));
	} else{
	$this->Text(05,25,$this->nbrtostring("structure","id",$STRUCTUREDX[1],"structure"));	$this->Text(240,25," LE : ".date('d-m-Y')); 
	}
	$this->Text(05,30,"SEMEP");
	$this->Text(05,35,"N ... /".date('Y'));
	
	$this->SetXY(5,35);
	if ($type == "V") {$this->cell(290,5,html_entity_decode(utf8_decode("RELEVE DES NAISSANCES VIVANTES ")),0,0,'C',0,0);} 
	if ($type == "M"){$this->cell(290,5,html_entity_decode(utf8_decode("RELEVE DES NAISSANCES MORT-NÉS ")),0,0,'C',0,0);} 
	...
	$this->SetXY(5,45);$this->cell(290,5,html_entity_decode(utf8_decode("Ref : ********************************  ")),0,0,'C',0,0);
	$h=55;
	$this->SetFillColor(200 );
	$this->SetXY(05,$h);
	$this->cell(10,10,html_entity_decode(utf8_decode("N°")),1,0,1,'C',0);
	$this->cell(30,10,html_entity_decode(utf8_decode("Date Naissance")),1,0,1,'C',0);
	$this->cell(50,10,html_entity_decode(utf8_decode("Nom_Prénom_Fils De")),1,0,1,'C',0);
	$this->cell(10,10,html_entity_decode(utf8_decode("Sexe")),1,0,1,'C',0);
	$this->cell(50,10,html_entity_decode(utf8_decode("Nom_Prénom_Mére ")),1,0,1,'C',0);
	$this->cell(40,10,html_entity_decode(utf8_decode("Wilaya de Residence")),1,0,1,'C',0);
	$this->cell(50,10,html_entity_decode(utf8_decode("Commune de Residence")),1,0,1,'C',0);
	$this->cell(46,10,html_entity_decode(utf8_decode("Adresse de Residence")),1,0,1,'C',0);
	$this->SetXY(05,$h+10); 
	$this->mysqlconnect();
    $this->SetFont('Arial', '',9, '', true);
	if ($type=='V') {$query = "SELECT * FROM naissance where DINS1 BETWEEN '$datejour1' AND '$datejour2' and STRUCTURED $STRUCTURED and VMN='V' order by  DINS1"; } 
	if ($type=='M') {$query = "SELECT * FROM naissance where DINS1 BETWEEN '$datejour1' AND '$datejour2' and STRUCTURED $STRUCTURED and VMN='M' order by  DINS1"; } 
	$resultat=mysql_query($query);
	...
	$this->cell(30,5,$this->dateUS2FR($row->DINS1),1,0,'C',0);
	if ($type=='V') {$this->cell(50,5,trim($row->NOM3).'_'.strtolower (trim($row->PRENOM5)).' [ '.strtolower (trim($row->PRENOM3)).' ]',1,0,'L',0);}
	if ($type=='M') {$this->cell(50,5,trim($row->NOM3).'_'.'xxxx'.' [ '.strtolower (trim($row->PRENOM3)).' ]',1,0,'L',0);}
	$this->cell(10,5,trim($row->SEXE5),1,0,'C',0);
	$this->cell(50,5,trim($row->NOM2).'_'.strtolower (trim($row->PRENOM2)),1,0,'L',0);
	$this->cell(40,5,$this->nbrtostring("wil","IDWIL",$row->WILAYA4,"WILAYAS"),1,0,'L',0);
	$this->cell(50,5,$this->nbrtostring("com","IDCOM",$row->COMMUNE4,"COMMUNE"),1,0,'L',0);
	$this->cell(46,5,trim($row->ADRESSE4),1,0,'C',0);
	$this->SetXY(5,$this->GetY()+5); 
	}
	$this->SetFillColor(200 );
	$this->SetFont('Arial', '',10, '', true);  
	$this->SetXY(5,$this->GetY());$this->cell(40,05,"Total",1,0,1,'C',0);	  
	$this->SetXY(45,$this->GetY());$this->cell(246,05,$totalmbr1." Naissance(s)",1,1,1,'C',0);				 
	...
	$this->SetXY(5,30);$this->cell(200,5,html_entity_decode(utf8_decode($this->dspfr.': '.$this->nbrtostring("wil","IDWIL",$this->nbrtostring("structure","id",$STRUCTURED,"idwil"),"WILAYAS"))),0,0,'C',1,0);
	...
	$this->SetXY(40,55);$this->cell(150,5,html_entity_decode(utf8_decode("A")),0,0,'C',0,0);
	$this->SetXY(40,65);$this->cell(150,5,html_entity_decode(utf8_decode("Monsieur le Directeur de la sante et de la population de la wilaya de ".$this->nbrtostring("wil","IDWIL",$this->nbrtostring("structure","id",$STRUCTURED,"idwil"),"WILAYAS"))),0,0,'C',0,0);
	$this->SetXY(5,85);$this->cell(200,10,html_entity_decode(utf8_decode($titre )),0,0,'C',1,0);
	...
	$this->SetXY(5+15,148);$this->cell(105,10,html_entity_decode(utf8_decode("Certificats de Naissance" )),0,0,'L',0,0);
	$this->SetXY(5+15+105,148);$this->cell(15,10,$this->valeurmois('naissance','DINS1',$datejour1,$datejour2,$EPH1),0,0,'C',0,0);
	...
	$this->SetXY(5+15,158);$this->cell(105,10,html_entity_decode(utf8_decode("Liste nominative des Naissances hospitaliers" )),0,0,'L',0,0);
	...
	$this->SetXY(5+15,168);$this->cell(105,10,html_entity_decode(utf8_decode("Rapport de la Natalité hospitalière" )),0,0,'L',0,0);
	$this->SetXY(5+15+105,168);$this->cell(15,10,html_entity_decode(utf8_decode("01" )),0,0,'C',0,0);
	$this->SetXY(5+30+105,168);$this->cell(65,10,html_entity_decode(utf8_decode("Natalité Hospitalière" )),0,0,'C',0,0);
	...
	$this->SetXY(5,70);$this->cell(200,5,html_entity_decode(utf8_decode("Natalité Intra-Hospitaliere")),0,0,'C',1,0);
	...
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("I-Distribution des Naissances  par L'âge de la mère")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("II-Distribution des Naissances  par parité")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("III-Distribution des Naissances  par Durée de gestation")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("IV-Distribution des Naissances  par Mode D'accouchement")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("V-Distribution des Naissances  par Mode d'expulsion selon la présentation")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("VI-Distribution des Naissances  par poids de naissance")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("VII-Distribution des Naissances  par score d'Apgar a la première minute")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("VIII-Distribution des Naissances  par mise au sein de l'enfant après la naissance")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("IX-")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("X-")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("XI-")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("XII-")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("XIII-")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("Distribution suivant l'heure de survenue ")),0,0,'L',1,0);
	// $this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("")),0,0,'L',1,0);
	$this->SetTextColor(0,0,0);
	$this->SetFont('Times', 'B', 11);
	}
	
	function modeaccouchmentf($colone1,$colone2,$datejour1,$datejour2,$STRUCTURED)
	{
	$this->mysqlconnect();
	$sql = " select * from naissance where  ($colone1 = '$colone2' )  and (DINS1 BETWEEN '$datejour1' AND '$datejour2') and (STRUCTURED $STRUCTURED ) ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$collecte=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $collecte;
	...
	function modeaccouchment($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$t1=$this->modeaccouchmentf('MAC','VB',$datejour1,$datejour2,"=".$STRUCTURED);
	$t2=$this->modeaccouchmentf('MAC','VH',$datejour1,$datejour2,"=".$STRUCTURED);
	$tt=$t1+$t2;
	if ($tt>0) {
	$t11=$t1+$t2;
	} else {
	...
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("IV-Distribution des Naissances  par Mode D'accouchement ")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("Mode D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Voie Basse Naturelle   ")),1,0,'L',0,0);$this->cell(66,10,$t1,1,0,'C',0,0);$this->cell(66,10,round (($t1*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Voie Haute Césarienne ")),1,0,'L',0,0);$this->cell(66,10,$t2,1,0,'C',0,0);$this->cell(66,10,round (($t2*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0);$this->cell(66,10,$tt,1,0,'C',1,0);$this->cell(66,10,'***',1,0,'C',1,0);
	}
	
	function modeexpulssion($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("V-Distribution des Naissances  par Mode d'expulsion selon la présentation")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("Mode D'accouchement ")),1,0,'L',1,0);$this->cell(54,10,html_entity_decode(utf8_decode("Voie Basse Naturelle ")),1,0,'L',1,0);$this->cell(54,10,html_entity_decode(utf8_decode("Voie Haute Césarienne")),1,0,'L',1,0);$this->cell(24,10,html_entity_decode(utf8_decode("Total")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Sommet   ")),1,0,'L',0,0);$this->cell(54,10,0,1,0,'C',0,0);$this->cell(54,10,0,1,0,'C',0,0);$this->cell(24,10,0,1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Transversale ")),1,0,'L',0,0);$this->cell(54,10,0,1,0,'C',0,0);$this->cell(54,10,0,1,0,'C',0,0);$this->cell(24,10,0,1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Siège ")),1,0,'L',0,0);$this->cell(54,10,0,1,0,'C',0,0);$this->cell(54,10,0,1,0,'C',0,0);$this->cell(24,10,0,1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0);$this->cell(54,10,0,1,0,'C',1,0);$this->cell(54,10,0,1,0,'C',1,0);$this->cell(24,10,0,1,0,'C',1,0);
	}
	
	function poidsnaissance($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$t1=$this->naissance('POIDS',0,499,$datejour1,$datejour2,"=".$STRUCTURED);
	$t2=$this->naissance('POIDS',500,999,$datejour1,$datejour2,"=".$STRUCTURED);
	$t3=$this->naissance('POIDS',1000,1499,$datejour1,$datejour2,"=".$STRUCTURED);
	$t4=$this->naissance('POIDS',1500,1999,$datejour1,$datejour2,"=".$STRUCTURED);
	$t5=$this->naissance('POIDS',2000,2499,$datejour1,$datejour2,"=".$STRUCTURED);
	$t6=$this->naissance('POIDS',2500,2999,$datejour1,$datejour2,"=".$STRUCTURED);
	$t7=$this->naissance('POIDS',3000,3499,$datejour1,$datejour2,"=".$STRUCTURED);
	$t8=$this->naissance('POIDS',3500,3999,$datejour1,$datejour2,"=".$STRUCTURED);
	$t9=$this->naissance('POIDS',4000,4499,$datejour1,$datejour2,"=".$STRUCTURED);
	$t10=$this->naissance('POIDS',4500,6000,$datejour1,$datejour2,"=".$STRUCTURED);
	$tt=$this->naissance('POIDS',0,6000,$datejour1,$datejour2,"=".$STRUCTURED);
	if ($tt>0) {
	$t11=$this->naissance('POIDS',0,6000,$datejour1,$datejour2,"=".$STRUCTURED);
	} else {
	$t11=1;
	}
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("VI-Distribution des Naissances  par poids de naissance ")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("Poids de naissance en grammes ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" < 500 ")),1,0,'C',0,0);    $this->cell(66,10,$t1,1,0,'C',0,0);$this->cell(66,10,round (($t1*100)/$t11,2),1,0,'C',0,0);
    $this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[500-1000[")),1,0,'C',0,0); $this->cell(66,10,$t2,1,0,'C',0,0);$this->cell(66,10,round (($t2*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[1000-1500[")),1,0,'C',0,0);$this->cell(66,10,$t3,1,0,'C',0,0);$this->cell(66,10,round (($t3*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[1500-2000[")),1,0,'C',0,0);$this->cell(66,10,$t4,1,0,'C',0,0);$this->cell(66,10,round (($t4*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[2000-2500[")),1,0,'C',0,0);$this->cell(66,10,$t5,1,0,'C',0,0);$this->cell(66,10,round (($t5*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[2500-3000[")),1,0,'C',0,0);$this->cell(66,10,$t6,1,0,'C',0,0);$this->cell(66,10,round (($t6*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[3000-3500[")),1,0,'C',0,0);$this->cell(66,10,$t7,1,0,'C',0,0);$this->cell(66,10,round (($t7*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[3500-4000[")),1,0,'C',0,0);$this->cell(66,10,$t8,1,0,'C',0,0);$this->cell(66,10,round (($t8*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[4000-4500[")),1,0,'C',0,0);$this->cell(66,10,$t9,1,0,'C',0,0);$this->cell(66,10,round (($t9*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" > 4500")),1,0,'C',0,0);    $this->cell(66,10,$t10,1,0,'C',0,0);$this->cell(66,10,round (($t10*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0);     $this->cell(66,10,$t11,1,0,'C',1,0);$this->cell(66,10,'***',1,0,'C',1,0);
	}
	
	
	
	function naissance($colone1,$colone2,$colone3,$datejour1,$datejour2,$STRUCTURED)
	{
	$this->mysqlconnect();
	$sql = " select * from naissance where  ($colone1 >=$colone2  and $colone1 <=$colone3)  and (DINS1 BETWEEN '$datejour1' AND '$datejour2') and (STRUCTURED $STRUCTURED ) ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$collecte=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $collecte;
	...
	function agemere($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$t1=$this->naissance('AGE2',0,14,$datejour1,$datejour2,"=".$STRUCTURED);
	$t2=$this->naissance('AGE2',15,19,$datejour1,$datejour2,"=".$STRUCTURED);
	$t3=$this->naissance('AGE2',20,24,$datejour1,$datejour2,"=".$STRUCTURED);
	$t4=$this->naissance('AGE2',25,29,$datejour1,$datejour2,"=".$STRUCTURED);
	$t5=$this->naissance('AGE2',30,34,$datejour1,$datejour2,"=".$STRUCTURED);
	$t6=$this->naissance('AGE2',35,39,$datejour1,$datejour2,"=".$STRUCTURED);
	$t7=$this->naissance('AGE2',40,44,$datejour1,$datejour2,"=".$STRUCTURED);
	$t8=$this->naissance('AGE2',45,49,$datejour1,$datejour2,"=".$STRUCTURED);
	$t9=$this->naissance('AGE2',50,54,$datejour1,$datejour2,"=".$STRUCTURED);
	$t10=$this->naissance('AGE2',55,59,$datejour1,$datejour2,"=".$STRUCTURED);
	$tt=$this->naissance('AGE2',0,100,$datejour1,$datejour2,"=".$STRUCTURED);
	if ($tt>0) {
	$t11=$this->naissance('AGE2',0,100,$datejour1,$datejour2,"=".$STRUCTURED);
	} else {
	$t11=1;
	}
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("I-Distribution des Naissances  par L'âge de la mère ")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("L'âge de la mère en années  ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement % ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" < 15 ")),1,0,'C',0,0); $this->cell(66,10,$t1,1,0,'C',0,0);$this->cell(66,10,round (($t1*100)/$t11,2),1,0,'C',0,0);
    $this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[15-20[")),1,0,'C',0,0);$this->cell(66,10,$t2,1,0,'C',0,0);$this->cell(66,10,round (($t2*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[20-25[")),1,0,'C',0,0);$this->cell(66,10,$t3,1,0,'C',0,0);$this->cell(66,10,round (($t3*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[25-30[")),1,0,'C',0,0);$this->cell(66,10,$t4,1,0,'C',0,0);$this->cell(66,10,round (($t4*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[30-35[")),1,0,'C',0,0);$this->cell(66,10,$t5,1,0,'C',0,0);$this->cell(66,10,round (($t5*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[35-40[")),1,0,'C',0,0);$this->cell(66,10,$t6,1,0,'C',0,0);$this->cell(66,10,round (($t6*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[40-45[")),1,0,'C',0,0);$this->cell(66,10,$t7,1,0,'C',0,0);$this->cell(66,10,round (($t7*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[45-50[")),1,0,'C',0,0);$this->cell(66,10,$t8,1,0,'C',0,0);$this->cell(66,10,round (($t8*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[50-55[")),1,0,'C',0,0);$this->cell(66,10,$t9,1,0,'C',0,0);$this->cell(66,10,round (($t9*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("> 55")),1,0,'C',0,0);   $this->cell(66,10,$t10,1,0,'C',0,0);$this->cell(66,10,round (($t10*100)/$t11,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0); $this->cell(66,10,$t11,1,0,'C',1,0);$this->cell(66,10,'***',1,0,'C',1,0);
	}
	
	function duregestation($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$t1=$this->naissance('SA',0,27,$datejour1,$datejour2,"=".$STRUCTURED);
	$t2=$this->naissance('SA',28,31,$datejour1,$datejour2,"=".$STRUCTURED);
	$t3=$this->naissance('SA',32,36,$datejour1,$datejour2,"=".$STRUCTURED);
	$t4=$this->naissance('SA',37,39,$datejour1,$datejour2,"=".$STRUCTURED);
	$t5=$this->naissance('SA',40,41,$datejour1,$datejour2,"=".$STRUCTURED);
	$t6=$this->naissance('SA',42,44,$datejour1,$datejour2,"=".$STRUCTURED);
	$tt=$this->naissance('SA',0,44,$datejour1,$datejour2,"=".$STRUCTURED);
	
	if ($tt>0) {
	$t7=$this->naissance('SA',0,44,$datejour1,$datejour2,"=".$STRUCTURED);
	} else {
	$t7=1;
	}
	
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("III-Distribution des Naissances  par Durée de gestation ")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("Durée de gestation en SA ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement % ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" < 28 ")),1,0,'C',0,0); $this->cell(66,10,$t1,1,0,'C',0,0);$this->cell(66,10,round (($t1*100)/$t7,2),1,0,'C',0,0);
    $this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[28-32[")),1,0,'C',0,0);$this->cell(66,10,$t2,1,0,'C',0,0);$this->cell(66,10,round (($t2*100)/$t7,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[32-37[")),1,0,'C',0,0);$this->cell(66,10,$t3,1,0,'C',0,0);$this->cell(66,10,round (($t3*100)/$t7,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[37-40[")),1,0,'C',0,0);$this->cell(66,10,$t4,1,0,'C',0,0);$this->cell(66,10,round (($t4*100)/$t7,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("[40-42[")),1,0,'C',0,0);$this->cell(66,10,$t5,1,0,'C',0,0);$this->cell(66,10,round (($t5*100)/$t7,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" > 42")),1,0,'C',0,0);  $this->cell(66,10,$t6,1,0,'C',0,0);$this->cell(66,10,round (($t6*100)/$t7,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0); $this->cell(66,10,$t7,1,0,'C',1,0);$this->cell(66,10,'***',1,0,'C',1,0);
	}
	
	function apgar($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$t1=$this->paritef('APGAR',0,$datejour1,$datejour2,"=".$STRUCTURED);
	$t2=$this->paritef('APGAR',1,$datejour1,$datejour2,"=".$STRUCTURED);
	$t3=$this->paritef('APGAR',2,$datejour1,$datejour2,"=".$STRUCTURED);
	$t4=$this->paritef('APGAR',3,$datejour1,$datejour2,"=".$STRUCTURED);
	$t5=$this->paritef('APGAR',4,$datejour1,$datejour2,"=".$STRUCTURED);
	$t6=$this->paritef('APGAR',5,$datejour1,$datejour2,"=".$STRUCTURED);
	$t7=$this->paritef('APGAR',6,$datejour1,$datejour2,"=".$STRUCTURED);
	$t8=$this->paritef('APGAR',7,$datejour1,$datejour2,"=".$STRUCTURED);
	$t9=$this->paritef('APGAR',8,$datejour1,$datejour2,"=".$STRUCTURED);
	$t10=$this->paritef('APGAR',9,$datejour1,$datejour2,"=".$STRUCTURED);
	$t11=$this->paritef('APGAR',10,$datejour1,$datejour2,"=".$STRUCTURED);
	$tt=$t1+$t2+$t3+$t4+$t5+$t6+$t7+$t8+$t9+$t10+$t11;
	if ($tt>0) {
	$t12=$tt;
	} else {
	$t12=1;
	}
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("VII-Distribution des Naissances  par score d'Apgar a la première minute")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("Score d'Apgar a la première minute ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 0 ")),1,0,'C',0,0);$this->cell(66,10,$t1,1,0,'C',0,0);$this->cell(66,10,round (($t1*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 1 ")),1,0,'C',0,0);$this->cell(66,10,$t2,1,0,'C',0,0);$this->cell(66,10,round (($t2*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 2 ")),1,0,'C',0,0);$this->cell(66,10,$t3,1,0,'C',0,0);$this->cell(66,10,round (($t3*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 3 ")),1,0,'C',0,0);$this->cell(66,10,$t4,1,0,'C',0,0);$this->cell(66,10,round (($t4*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 4 ")),1,0,'C',0,0);$this->cell(66,10,$t5,1,0,'C',0,0);$this->cell(66,10,round (($t5*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 5 ")),1,0,'C',0,0);$this->cell(66,10,$t6,1,0,'C',0,0);$this->cell(66,10,round (($t6*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 6 ")),1,0,'C',0,0);$this->cell(66,10,$t7,1,0,'C',0,0);$this->cell(66,10,round (($t7*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 7 ")),1,0,'C',0,0);$this->cell(66,10,$t8,1,0,'C',0,0);$this->cell(66,10,round (($t8*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 8 ")),1,0,'C',0,0);$this->cell(66,10,$t9,1,0,'C',0,0);$this->cell(66,10,round (($t9*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 9 ")),1,0,'C',0,0);$this->cell(66,10,$t10,1,0,'C',0,0);$this->cell(66,10,round (($t10*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode(" 10 ")),1,0,'C',0,0);$this->cell(66,10,$t11,1,0,'C',0,0);$this->cell(66,10,round (($t11*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0);$this->cell(66,10,$t12,1,0,'C',1,0);$this->cell(66,10,'***',1,0,'C',1,0);
	}
	
	
	function paritef($colone1,$colone2,$datejour1,$datejour2,$STRUCTURED)
	{
	$this->mysqlconnect();
	$sql = " select * from naissance where  ($colone1 = $colone2 )  and (DINS1 BETWEEN '$datejour1' AND '$datejour2') and (STRUCTURED $STRUCTURED ) ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	...
	function parite($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$t1=$this->paritef('PARITE',0,$datejour1,$datejour2,"=".$STRUCTURED);
	$t2=$this->paritef('PARITE',1,$datejour1,$datejour2,"=".$STRUCTURED);
	$t3=$this->paritef('PARITE',2,$datejour1,$datejour2,"=".$STRUCTURED);
	$t4=$this->paritef('PARITE',3,$datejour1,$datejour2,"=".$STRUCTURED);
	$t5=$this->paritef('PARITE',4,$datejour1,$datejour2,"=".$STRUCTURED);
	$t6=$this->paritef('PARITE',5,$datejour1,$datejour2,"=".$STRUCTURED);
	$t7=$this->paritef('PARITE',6,$datejour1,$datejour2,"=".$STRUCTURED);
	$t8=$this->paritef('PARITE',7,$datejour1,$datejour2,"=".$STRUCTURED);
	$t9=$this->paritef('PARITE',8,$datejour1,$datejour2,"=".$STRUCTURED);
	$t10=$this->paritef('PARITE',9,$datejour1,$datejour2,"=".$STRUCTURED);
	$t11=$this->paritef('PARITE',10,$datejour1,$datejour2,"=".$STRUCTURED);
	$t12=$t1+$t2+$t3+$t4+$t5+$t6+$t7+$t8+$t9+$t10+$t11;
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("II-Distribution des Naissances  par parité")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("la parité")),1,0,'L',1,0);      $this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement % ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("0")),1,0,'C',0,0);             $this->cell(66,10,$t1,1,0,'C',0,0);$this->cell(66,10,round (($t1*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("1")),1,0,'C',0,0);  $this->cell(66,10,$t2,1,0,'C',0,0);$this->cell(66,10,round (($t2*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("2")),1,0,'C',0,0);              $this->cell(66,10,$t3,1,0,'C',0,0);$this->cell(66,10,round (($t3*100)/$t12,2),1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("3")),1,0,'C',0,0);              $this->cell(66,10,$t4,1,0,'C',0,0);$this->cell(66,10,round (($t4*100)/$t12,2),1,0,'C',0,0);
	...
	function sein($datejour1,$datejour2,$EPH1,$STRUCTURED)
	{
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("VII-Distribution des Naissances  par mise au sein de l'enfant après la naissance")),0,0,'C',1,0);
	$this->SetXY(5,$this->GetY()+15);$this->cell(68,10,html_entity_decode(utf8_decode("mise au sein")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Nombre D'accouchement ")),1,0,'L',1,0);$this->cell(66,10,html_entity_decode(utf8_decode("Pourcentage D'accouchement ")),1,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Néant")),1,0,'C',0,0);$this->cell(66,10,0,1,0,'C',0,0);$this->cell(66,10,0,1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Immédiate")),1,0,'C',0,0);$this->cell(66,10,0,1,0,'C',0,0);$this->cell(66,10,0,1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Tardive")),1,0,'C',0,0);$this->cell(66,10,0,1,0,'C',0,0);$this->cell(66,10,0,1,0,'C',0,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(68,10,html_entity_decode(utf8_decode("Total ")),1,0,'L',1,0);$this->cell(66,10,0,1,0,'C',1,0);$this->cell(66,10,0,1,0,'C',1,0);
	}
	...
	function valeurmoisdeceshc($TBL,$COLONE1,$DATEJOUR1,$DATEJOUR2,$STRUCTURED,$wilaya) 
	{
	$this->mysqlconnect();
	$sql = " select * from $TBL where  ($COLONE1 BETWEEN '$DATEJOUR1' AND '$DATEJOUR2')  and  WILAYAR!= $wilaya  and  (STRUCTURED $STRUCTURED)";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$OP=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $OP;
	}
	function valeurmoisdecest($SRS,$TBL,$COLONE1,$COLONE2,$DATEJOUR1,$DATEJOUR2,$VALEUR2,$STR,$STRUCTURED,$wilaya) 
	{
	$this->mysqlconnect();
	$sql = " select * from $TBL where $COLONE1 BETWEEN '$DATEJOUR1' AND '$DATEJOUR2' and (STRUCTURED $STRUCTURED) and  WILAYAR = $wilaya  ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$OP=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $OP;
	...
	{    
	   ...
	   //non coriger  probleme des hors commune 
		...
		// $this->cell(20,5,$this->valeurmoisdeceshc('deceshosp','DINS',$datejour1,$datejour2,$STRUCTURED,$STRUCTUREDX[1]),1,0,'C',1,0);
		...
		// $this->cell(20,5,$this->valeurmoisdecest('','deceshosp','DINS','COMMUNER',$datejour1,$datejour2,'','',$STRUCTURED,$STRUCTUREDX[1]),1,0,'C',1,0);	  
	    // $this->cell(20,5,round(($this->valeurmoisdecest('','deceshosp','DINS','COMMUNER',$datejour1,$datejour2,'','',$STRUCTURED,$STRUCTUREDX[1])*1000)/round($rs1['total1'],3),3),1,0,'C',1,0);	  
	}
	
	function tblparwilaya($dnrdon,$datejour1,$datejour2,$STRUCTURED) 
	{    
		$this->SetFont('Times', 'B', 10);$h=35;
		...
		while($row=mysql_fetch_object($resultat))
		{
			...
			$this->cell(20,4,round( ($this->valeurmoisdeces('','deceshosp','DINS','WILAYAR',$datejour1,$datejour2,trim($row->IDWIL),'',$STRUCTURED)*1000)/$row->POPULATION ,3),1,0,'L',0);
			$this->SetXY(8,$this->GetY()+4); 
		}
		$this->mysqlconnect();
		$req="SELECT SUM(SUPER) AS total FROM wil  ";
		$query1 = mysql_query($req);   
		$rs = mysql_fetch_assoc($query1);
		$req1="SELECT SUM(POPULATION) AS total1 FROM wil  ";
		$query11 = mysql_query($req1);   
		$rs1 = mysql_fetch_assoc($query11);
		$req2="SELECT WILAYAR,STRUCTURED FROM deceshosp where STRUCTURED".$STRUCTURED;//
		$query12 = mysql_query($req2);   
		$rs2 = mysql_fetch_assoc($query12);
		$totalmbr2=mysql_num_rows($query12);
		
		$this->SetXY(8,$this->GetY());$this->cell(15,5,"Total",1,0,'C',1,0);	  
		$this->cell(90,5,($totalmbr1)."  Wilayas",1,0,'C',1,0);	  
		$this->cell(20,5,round($rs['total'],2),1,0,'C',1,0);
		$this->cell(30,5,round($rs1['total1'],2),1,0,'C',1,0);
		$this->cell(20,5,$totalmbr2,1,0,'C',1,0);
		$this->cell(20,5,(round($rs['total'],2)*1000)/round($rs1['total1'],2),1,0,'C',1,0);
	}
	...
	{
	$this->mysqlconnect();
	$sql = " select * from deceshosp where STRUCTURED $STRUCTURED and (WILAYAR = $WILAYAR) and   (DINS BETWEEN '$DATEJOUR1' AND '$DATEJOUR2') ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$OP=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $OP;
	...
	if($x >=1  and $x<=10){$this->SetDrawColor(255,0,0);$this->SetFillColor(0,250,255);}//jaune
