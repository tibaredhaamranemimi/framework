...
//******************************************************************************************************************************************************//	
	...
	$this->SetXY(5,40);$this->cell(100,5,"Service de la prevention",0,0,'L',0,0);$this->SetXY(155,40);$this->cell(50,5,"Le : ".date('d-m-Y'),0,0,'L',0,0);
	...
	$this->SetXY(5+15,148);$this->cell(105,10,html_entity_decode(utf8_decode("Certificats de décès" )),0,0,'L',0,0);
	$this->SetXY(5+15+105,148);$this->cell(15,10,$this->valeurmois('deceshosp','DINS',$datejour1,$datejour2,'IS NOT NULL'),0,0,'C',0,0);
...
//******************************************************************************************************************************************************//
	...
	$this->SetXY(5,90);$this->cell(200,5,"Total Wilaya",0,0,'C',1,0);
	$this->SetTextColor(0,0,0);$this->SetFont('Times', 'B', 10);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("Sommaire")),0,0,'L',1,0);
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("0-Distribution des décès par Etablissement")),0,0,'L',1,0);
	...
	$this->SetXY(5,$this->GetY()+10);$this->cell(200,10,html_entity_decode(utf8_decode("Distribution des décès suivant l'heure de survenue ")),0,0,'L',1,0);
...
//******************************************************************************************************************************************************//
	...
	function dspnbretab($datejour1,$datejour2,$STRUCTURED,$EPH1,$idwil)
	{
	$h=100;
	$this->SetXY(05,$h);$this->cell(10,10,"id",1,0,1,'C',0);$this->cell(130,10,"Etablissements",1,0,1,'C',0);$this->cell(30,10,"Nbr Deces",1,0,1,'C',0);$this->cell(30,10,"% Deces",1,0,1,'C',0);
	$this->SetXY(05,$h+10); 
	$this->mysqlconnect();
	...
	$this->cell(130,5,$row->structure,1,0,'L',0);
	$this->cell(30,5,$this->dspnbr($datejour1,$datejour2,"=".$row->id),1,0,'C',0);
	$this->cell(30,5,round((($this->dspnbr($datejour1,$datejour2,"=".$row->id)*100) / $this->dspnbr($datejour1,$datejour2,$EPH1)),2),1,0,'C',0);
	$this->SetXY(5,$this->GetY()+5); 
	}
	$this->cell(140,10,"Total Etablissements",1,0,1,'C',0);
	$this->cell(30,10,$this->dspnbr($datejour1,$datejour2,$EPH1),1,0,'C',1,0);
	$this->cell(30,10,'100 %',1,0,'C',1,0);
	}
	
//******************************************************************************************************************************************************//
    function nbrservicemf($nbrservice,$datejour1,$datejour2,$SEX)
	{
	$this->mysqlconnect();
	$sql = " select * from deceshosp where SERVICEHOSPIT=$nbrservice  and SEX='$SEX' and (DINS BETWEEN '$datejour1' AND '$datejour2')";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$collecte=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $collecte;
	...
	{ 
		...
		$TA=array();$TAM=array();$TAF=array();
		for ($i = 1; $i <= 21; $i+=1)
		{
		$TA[$i]=$this->nbrservice($i,$datejour1,$datejour2,$eph);
		$TAM[$i]=$this->nbrservicemf($i,$datejour1,$datejour2,"M");
		$TAF[$i]=$this->nbrservicemf($i,$datejour1,$datejour2,"F");
		}
		$TA21x=array_sum($TA);if ($TA21x>0) {$TA21=array_sum($TA);}else{$TA21=1;} 
		$datamf = $TA;
		...
		$this->SetXY(5,35+7+5);$this->cell(10,5,"Num",1,0,'C',1,0);                  $this->cell(50,5,"Service",1,0,'L',1,0);                                                 $this->cell(10,5,"M",1,0,'C',1,0);              $this->cell(10,5,"F",1,0,'C',1,0);                   $this->cell(10,5,"T",1,0,'C',1,0);               $this->cell(15,5,"Tx %",1,0,'C',1,0);
		$this->SetFont( 'Arial', 'B', 8 );
		$this->mysqlconnect();
		$query="SELECT * FROM servicedeces";
		$resultat=mysql_query($query);
		while($row=mysql_fetch_object($resultat))
		{
		$this->SetXY(5,$this->GetY()+5);$this->cell(10,5,$row->id,1,0,'L',0);        $this->cell(50,5,html_entity_decode(utf8_decode($row->service)),1,0,'L',0);               $this->cell(10,5,$TAM[$row->id],1,0,'C',0);    $this->cell(10,5,$TAF[$row->id],1,0,'C',0);          $this->cell(10,5,$TA[$row->id],1,0,'C',0);        $this->cell(15,5,round(($TA[$row->id]*100)/$TA21,2),1,0,'C',0);
		}
		$this->SetFont('Arial', 'B', 10);
        $this->SetXY(5,$this->GetY()+5); $this->cell(60,5,"Total",1,0,'L',1,0);                                                                                                $this->cell(10,5,array_sum($TAM),1,0,'C',1,0); $this->cell(10,5,array_sum($TAF),1,0,'C',1,0);       $this->cell(10,5,$TA21,1,0,'C',1,0);              $this->cell(15,5,round(($TA21*100)/$TA21,2),1,0,'C',1,0);						
		$mas=array_sum($TAM);
		$fem=array_sum($TAF);
		$pie2 = array(
		"x"  => 135, 
		...
		"t2" => "F");
		$this->pie2($pie2);
		$this->SetXY(5,165);$this->cell(285,5,html_entity_decode(utf8_decode("1-Repartition des deces par service : ")),0,0,'L',0);
	    rsort($datamf);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("la proportion des décès la plus élevée est : ".round($datamf[0]*100/$TA21,2)."%")),0,0,'L',0);
	    sort($datamf);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("la proportion des décès la moins élevée est : ".round($datamf[0]*100/$TA21,2)."%")),0,0,'L',0);
		$this->SetXY(5,$this->GetY()+10);$this->cell(285,5,html_entity_decode(utf8_decode("2-Répartition des décès par sexe : ")),0,0,'L',0);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("La répartition des ".$TA21." décès enregistrés montre que :")),0,0,'L',0);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode(round(($mas/$TA21)*100,2)."% des décès touche les hommes. ")),0,0,'L',0);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode(round(($fem/$TA21)*100,2)."% des décès touche les femmes. ")),0,0,'L',0);
	    if ($fem > 0){$fem=$fem;}else{$fem=1;}
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("avec un sexe ratio de ".round(($mas/$fem),2))),0,0,'L',0);
		$this->barservice(135,150,$TA[1],$TA[2],$TA[3],$TA[4],$TA[5],$TA[6],$TA[7],$TA[8],$TA[9],$TA[10],$TA[11],$TA[12],$TA[13],$TA[14],$TA[15],$TA[16],$TA[17],$TA[18],$TA[19],$TA[20],$titre); 	
		}

//******************************************************************************************************************************************************//
        
        function nbrserviced($nbrservice,$datejour1,$datejour2)
		{
		$this->mysqlconnect();
		$sql = " select * from deceshosp where DUREEHOSPIT=$nbrservice  and (DINS BETWEEN '$datejour1' AND '$datejour2')";
		$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
		$collecte=mysql_num_rows($requete);
		mysql_free_result($requete);
		return $collecte;
        ...
        function nbrservicedMF($nbrservice,$datejour1,$datejour2,$sex)
		{
		$this->mysqlconnect();
		$sql = " select * from deceshosp where DUREEHOSPIT=$nbrservice and SEX='$sex' and (DINS BETWEEN '$datejour1' AND '$datejour2')";
		$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
		$collecte=mysql_num_rows($requete);
		mysql_free_result($requete);
		return $collecte;
		...
		for ($i = 0; $i <= 20; $i+=1)
		{
		$TA[$i]=$this->nbrserviced($i,$datejour1,$datejour2);   
		$TAM[$i]=$this->nbrservicedMF($i,$datejour1,$datejour2,"M");
		$TAF[$i]=$this->nbrservicedMF($i,$datejour1,$datejour2,"F");
		}
		$TA21x=array_sum($TA);if ($TA21x>0) {$TA21=array_sum($TA);}else{$TA21=1;} 
		$datamf = $TA;
		$this->SetXY(5,25+10);$this->cell(285,5,html_entity_decode(utf8_decode("Cette étude a porté sur ".$TA21." décès survenus durant la periode du ".$this->dateUS2FR($datejour1)." au ".$this->dateUS2FR($datejour2)." au niveau de 36 communes ")),0,0,'L',0);
		...
		$this->SetXY(5,35+7+5);$this->cell(60,5,"Nombre de jours",1,0,'C',1,0);     $this->cell(10,5,"M",1,0,'C',1,0);$this->cell(10,5,"F",1,0,'C',1,0);           $this->cell(10,5,"T",1,0,'C',1,0);   $this->cell(15,5,"Tx %",1,0,'C',1,0);
		for ($i = 0; $i <= 20; $i+=1)
		{
		$this->SetXY(5,$this->GetY()+5);$this->cell(60,5,$i,1,0,'C',0);             $this->cell(10,5,$TAM[$i],1,0,'C',0);$this->cell(10,5,$TAF[$i],1,0,'C',0);     $this->cell(10,5,$TA[$i],1,0,'C',0); $this->cell(15,5,round(($TA[$i]*100)/$TA21,2),1,0,'C',0);
		}
        $this->SetXY(5,$this->GetY()+5); $this->cell(60,5,"Total",1,0,'L',1,0);    	$this->cell(10,5,array_sum($TAM),1,0,'C',1,0); $this->cell(10,5,array_sum($TAF),1,0,'C',1,0);       $this->cell(10,5,$TA21,1,0,'C',1,0);              $this->cell(15,5,round(($TA21*100)/$TA21,2),1,0,'C',1,0);										
		$mas=array_sum($TAM);
		$fem=array_sum($TAF);
		...
		"t2" => "F");
		$this->pie2($pie2);
		$this->SetXY(5,165);$this->cell(285,5,html_entity_decode(utf8_decode("1-Repartition des deces par Durée D'hospitalisation : ")),0,0,'L',0);
	    rsort($datamf);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("la proportion des décès la plus élevée est : ".round($datamf[0]*100/$TA21,2)."%")),0,0,'L',0);
	    sort($datamf);
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("la proportion des décès la moins élevée est : ".round($datamf[0]*100/$TA21,2)."%")),0,0,'L',0);
	    ...
	    if ($fem > 0){$fem=$fem;}else{$fem=1;}
	    $this->SetXY(5,$this->GetY()+5);$this->cell(285,5,html_entity_decode(utf8_decode("avec un sexe ratio de ".round(($mas/$fem),2))),0,0,'L',0);
		$this->barservice(135,150,$TA[0],$TA[1],$TA[2],$TA[3],$TA[4],$TA[5],$TA[6],$TA[7],$TA[8],$TA[9],$TA[10],$TA[11],$TA[12],$TA[13],$TA[14],$TA[15],$TA[16],$TA[17],$TA[18],$TA[19],$titre);	
	    }
//******************************************************************************************************************************************************//
	...
	$this->Text(05,30,html_entity_decode(utf8_decode("Service Prévention")));$this->Text(240,30," LE : ".date('d-m-Y')); 
	...
	$this->cell(126,5,'('.html_entity_decode(utf8_decode($this->nbrtostring("CIM","row_id",$row->CODECIM,'diag_nom'))).')_'.html_entity_decode(utf8_decode($this->nbrtostring("CIM","row_id",$row->CODECIM,'diag_cod'))),1,0,'L',0);
	}else{
	...
	function naissance($datejour1,$datejour2,$SEXEDNR,$STRUCTURED,$VMN)
	{
	$this->mysqlconnect();
	//$sql = " select * from naissance where  ($colone1 >=$colone2  and $colone1 <=$colone3)  and            ";
	$sql = " select * from naissance where (DINS1 BETWEEN '$datejour1' AND '$datejour2') and (SEXE5='$SEXEDNR' and STRUCTURED $STRUCTURED ) and VMN='$VMN' ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$collecte=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $collecte;
	...
	function valeurmoisdeces($SRS,$TBL,$COLONE1,$COLONE2,$DATEJOUR1,$DATEJOUR2,$VALEUR2) 
	{
	$this->mysqlconnect();
	...
	{
	$this->mysqlconnect();
	$sql = " select * from $TBL where  ($COLONE1 BETWEEN '$DATEJOUR1' AND '$DATEJOUR2')  and  WILAYAR!= $wilaya ";
	$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
	$OP=mysql_num_rows($requete);
	mysql_free_result($requete);
	return $OP;
	...
	{    
		...
		while($row=mysql_fetch_object($resultat))
		{
			...
			$this->cell(20,4,$this->valeurmoisdeces('','deceshosp','DINS','COMMUNER',$datejour1,$datejour2,trim($row->IDCOM),''),1,0,'L',0);
			$this->cell(20,4,round(($this->valeurmoisdeces('','deceshosp','DINS','COMMUNER',$datejour1,$datejour2,trim($row->IDCOM),'')*1000)/$row->POPULATION,3),1,0,'L',0);
	...
	function tblparwilaya($dnrdon,$datejour1,$datejour2) 
	{    
		...
		while($row=mysql_fetch_object($resultat))
		{
			...
			$this->cell(20,4,trim($row->SUPER),1,0,'L',0);
			$this->cell(30,4,trim($row->POPULATION),1,0,'L',0);
			$this->cell(20,4,$this->valeurmoisdeces('','deceshosp','DINS','WILAYAR',$datejour1,$datejour2,trim($row->IDWIL),''),1,0,'L',0);
			$this->cell(20,4,round( ($this->valeurmoisdeces('','deceshosp','DINS','WILAYAR',$datejour1,$datejour2,trim($row->IDWIL),'')*1000)/$row->POPULATION ,3),1,0,'L',0);
			$this->SetXY(8,$this->GetY()+4); 
		}
		$this->mysqlconnect();
		...
		$query11 = mysql_query($req1);   
		$rs1 = mysql_fetch_assoc($query11);
		$req2="SELECT WILAYAR,STRUCTURED FROM deceshosp ";//
